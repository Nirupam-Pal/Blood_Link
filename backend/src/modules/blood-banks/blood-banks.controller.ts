...
export class BloodBanksController {
  ...
  @ApiResponse({
    status: 201,
    description: 'Blood Bank registered sucessfully',
  })
  @ApiResponse({
    status: 400,
    description: 'Email OTP verification pending or invalid inout',
  })
  @ApiResponse({
    status: 409,
    description: 'License number o email already exixts',
  })
  ...
  @Get('profile')
  async getBloodBank(@CurrentBloodBank() bloodBank: BloodBank) {
  ...
  @ApiOperation({
    summary:
      'Search active blood banks by state and optional filters (Protected)',
  })
  @ApiBody({ type: SearchBloodBankDto })
  @ApiResponse({
    status: 200,
    description: 'Matching blood banks retrieved successfully.',
  })
  @ApiResponse({
    status: 401,
  ...
  async updateInventory(
    ...
    return {
      message: 'Inventory updated successfully for specified blood groups',
      data,
    };
  }
  ...
  @Delete('profile')
  @Roles(Role.BLOOD_BANK)
  async deleteProfile(@CurrentBloodBank('id') bankId: string) {
